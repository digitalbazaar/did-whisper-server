...
const whisperClient = require('did-whisper');
...
app.post('/whisper', parseJSON, (req, res) => saveWhisper(req, res, client));
app.post('/whisper/encrypt', parseForm, (req, res) => encryptMessage(req, res, client));
...
function dispatchWhisper(req, res, whisper) {
  if(req.accepts('text/html')) { // requested in the browser
...
function saveWhisper(req, res, client, whisper=req.body) {
  ...
  return client.set(id, value, 'EX', whisper.expiration || ONE_WEEK)
    .then(() => {
      ...
      if(req.headers['content-type'] === 'application/x-www-form-urlencoded') {
        ...
        return res.redirect(createdUrl);
      }
...
function encryptMessage(req, res, client) {
  const form = req.body;

  return whisperClient
    .encryptForDid(form.did, form.message, form.expiration)
    .then(whisper => saveWhisper(req, res, client, whisper))
    .catch(error => sendError(res, error));
}
